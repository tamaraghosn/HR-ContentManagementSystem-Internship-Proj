...
const AddEditJobTtile = (props) => {
  ...
  async function fetchData() {
    ...
    if (props.type === "edit") {
      try {
        responseItem = await axios.get(`jobtitles/${id}`);
        // setItem({
  ...
  return (
    <Page
      ...
      backAction={{ onAction: () => navigate("/admin/jobtitles") }}
      narrowWidth
    >
  ...
  function handleSave() {
    if (!item.depName) {
      ...
      // props.type === "add"
      //   ? axios
      //       .post(`jobtitles`, bodyObj)
      //       .then((result) => {
      //         navigate("/admin/jobtitles");
      //       })
      //       .catch((err) => console.log(err))
      //   : axios
      //       .patch(`jobtitles/${id}`, bodyObj)
      //       .then((result) => {
      //         toggleActive();
      //         setIsSaving(false);
      //       })
