...
const Speciality = () => {
  ...
  const handleAddSpeciality = () => {
    navigate("/admin/specialities/new");
  };
  ...
  async function fetchData() {
    let responseSpeciality = "";
    let responseFeatures = [];

    try {
      responseSpeciality = await axios.get(
        `/specialities?per_page=${perPage}&page=${
      ...
      setPageObject({
        ...pageObject,
        tableItems: responseSpeciality.data.data.data.map((item, index) => [
        ...
        isLoading: false,
        isListEmpty: !responseSpeciality.data.data.data.length ? true : false,
        tableTotalPages: responseSpeciality.data.data.total,
      });
    } catch (error) {
      console.log(error);
    }
  ...
  const [toastContent, setToastContent] = useState(
    "This speciality has been successfully deleted"
  );
  ...
  return (
    <Page
      title="Speciality"
      primaryAction={
        <Button variant="primary" onClick={handleAddSpeciality}>
          Add Speciality
        </Button>
      }
    >
