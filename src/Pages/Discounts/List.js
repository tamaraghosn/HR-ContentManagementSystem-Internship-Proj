...
const Discounts = () => {
  ...
  async function fetchData() {
    ...
    try {
      responseDiscounts = await axios.get(
        `/discounts?per_page=${perPage}&page=${refBoolPage.current ? page : 1}${
          queryValue ? `&filter[global_search]=${queryValue}` : ""
        }`
      );
      setPageObject({
        ...pageObject,
        tableItems: responseDiscounts.data.data.data.map((item, index) => [
          item?.name && item.name,
          item.is_active ? (
        ...
        isLoading: false,
        isListEmpty: !responseDiscounts.data.data.data.length ? true : false,
        tableTotalPages: responseDiscounts.data.data.total,
      });
    } catch (error) {
      console.log(error);
    }
  ...
  const [toastContent, setToastContent] = useState(
    "This discount has been successfully deleted"
  );
  ...
  return (
    <Page
      title="Discounts"
      primaryAction={
        <Button primary onClick={handleAddDiscount}>
          Add Discount
        </Button>
      }
    >
